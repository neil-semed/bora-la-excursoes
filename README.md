...
- **Usuários** (tela nova, só Admin, no menu "🔑 Usuários"): lista todo mundo que já acessou o sistema pelo menos
  uma vez, com um botão **Editar** para definir o **perfil** (Admin/Escola/Pedagogia/Motorista) e, conforme o
  perfil escolhido, a **unidade escolar** ou o **motorista vinculado** — tudo pela tela, sem precisar mais rodar
  `update profiles set role = ...` no SQL Editor do Supabase a cada pessoa nova. Importante: essa tela só edita
  logins que **já existem** (ou seja, que já fizeram o primeiro login, o que cria a linha automaticamente com
  perfil "Escola"); criar a conta em si (e-mail + senha) continua sendo feito no painel do Supabase em
  **Authentication → Users → Add user** — ver seção 3 abaixo.
...
- **Em desenho, ainda não incluído nesta versão** (conversamos sobre isso e vou apresentar os esboços antes de
  programar): a tela de **Validações** da Escola (upload do projeto pedagógico, integrado ao Google Drive) e da
  Pedagogia (parecer + encaminhamento entre os 6 setores pedagógicos), e a tela final de "solicitação liberada
  para escala" depois da validação pedagógica completa.
