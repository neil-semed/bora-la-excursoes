...
```
Escola solicita  →  Pedagogia aprova/recusa  →  Admin aprova e atribui veículo/motorista  →  Motorista inicia e conclui a viagem
...
1. No projeto Supabase: **Project Settings → API**
2. Copie a **Project URL** e a chave **anon public**
3. Abra o app (local ou no GitHub Pages), clique em **"⚙️ Configurar Supabase"** na tela de login, cole os dois valores e clique em Salvar
4. A página recarrega e a partir daí o app passa a usar o banco de dados real (some o aviso de "modo demonstração")

Essa configuração fica salva no navegador de quem acessa (não precisa repetir toda hora, só na primeira vez em cada dispositivo/navegador).
...
├── supabase/
│   └── schema.sql         → script único que cria todo o banco de dados
└── README.md              → este guia
```
