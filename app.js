...
const DEMO_PROFILES = [
  { id: 'demo-admin', email: 'admin@teste.com', full_name: 'Admin Teste', role: 'admin', school_id: null, driver_id: null, setor_pedagogico: null },
  { id: 'demo-escola', email: 'escola@teste.com', full_name: 'Escola Teste', role: 'escola', school_id: 's1', driver_id: null, setor_pedagogico: null },
  { id: 'demo-pedagogia', email: 'pedagogia@teste.com', full_name: 'Pedagogia Teste', role: 'pedagogia', school_id: null, driver_id: null, setor_pedagogico: 'ensino_fundamental' },
  { id: 'demo-motorista', email: 'motorista@teste.com', full_name: 'Motorista Teste', role: 'motorista', school_id: null, driver_id: 'd1', setor_pedagogico: null },
];
...
function saveDemoData() {
  sessionStorage.setItem('demoData', JSON.stringify({ schools, vehicles, drivers, agenda, cooperativas, appSettings, demoProfiles }));
}
...
function renderUsuarios() {
  ...
  tbody.innerHTML = allProfiles
    ...
    <tr class="hover:bg-slate-50">
      ...
      <td class="px-4 py-3 text-sm"><button onclick="openUserModal('${p.id}')" class="text-xs text-emerald-600 hover:text-emerald-800">Editar</button></td>
    </tr>`;
    }).join('');
}
...
function renderAgenda() {
  ...
  tbody.innerHTML = filtered
    ...
    .map((a) => {
      ...
      const validadaCompletamente = ['approved', 'in_transit', 'completed'].includes(a.status);
      // Deriva "precisa de ATF" direto da cidade (em vez de só confiar na coluna atf_status,
      // que podia ficar desatualizada em viagens aprovadas antes desta correção).
      const cidadeForaDeNL = a.city && !a.city.toLowerCase().includes('nova lima');
      const precisaCooperativa = cidadeForaDeNL || a.atf_status === 'emitida' || (a.pca_count || 0) > 0;
      ...
      if (role === 'admin' && validadaCompletamente && precisaCooperativa) {
  ...
  tbody.innerHTML = minhas
    ...
    .map((a) => {
      ...
      if (a.doc_status === 'correcoes' && a.doc_parecer_comentario) {
        parecerCell = `<span class="text-xs text-amber-700">✏️ ${a.doc_parecer_comentario}</span>`;
      } else if (a.doc_status === 'rejeitado' && a.doc_parecer_comentario) {
        parecerCell = `<span class="text-xs text-red-700">🚫 ${a.doc_parecer_comentario}</span>`;
      } else if (a.doc_status === 'aceito') {
        parecerCell = '<span class="text-xs text-emerald-700">✅ Aprovado</span>';
      }
      return `
    <tr class="hover:bg-slate-50">
...
async function logDocHistory(excursionId, evento, setorOrigem, setorDestino, comentario) {
  if (sb) {
    await sb.from('excursion_doc_history').insert([{
      ...
      comentario: comentario || null, por: currentUser.id,
    }]);
  }
